...
## Feedback
Public/accountless but IDENTIFIED per-session feedback via QR code / link, open from the session's configured pre-start window until a configurable period after it ends (default 2h). Name and email are stored and visible to authorised moderators; one response per email/session is allowed. Feedback never changes attendance or issues a certificate. Departments customise rating/text fields. Moderators can request an optional privacy-processed AI summary: identity columns are omitted, known names are stripped, comments are fenced as untrusted, and welfare/safety content is kept for human review. Releasing feedback to accepted teachers sends aggregate ratings only, never respondent identities or raw comments, and withholds detail below five responses. Moderators can publish human-authored "You said, we did" entries.

## Certificates
PDF certificates for attendees and teachers require the current FINALIZED attendance revision and PRESENT/LATE status; a teacher certificate also requires an ACCEPTED assignment. Feedback is irrelevant to eligibility. Codes are publicly checked at /verify and show VALID, LEGACY, or REVOKED. Reopening attendance revokes current valid certificates and a corrected final revision can issue replacements.

## Session documents and activity
Every session has a Documents tab. Moderators and accepted registered teachers can upload PDF, DOCX, and PPTX files up to 25 MiB into private storage. PDFs can be viewed in the authenticated browser; Office files download and are basic package-validated, not antivirus-certified or converted in a webviewer. The manage Activity Log records governed attendance, certificate, teacher-report, and document events; it is not a complete security/read audit.
