        ...
        <div className="overflow-x-auto border border-gray-300 bg-white">
          <table className="w-full min-w-[760px] border-collapse text-left text-xs">
            ...
            <tbody>
              ...
              <ServiceRow service={config.aiProvider} purpose="Optional LLM, private learning-document processing, and provider-hosted authoritative web research" data="Purpose-limited session metadata, assistant messages, processed feedback, private PDF/DOCX/PPTX contents sent on moderator recap generation, document-derived search queries, public result URLs/titles, and generated recap scripts" status={config.aiEnabled ? 'Enabled' : 'Disabled'} />
              <ServiceRow service={config.speechProvider} purpose="Optional text-to-speech for moderator-reviewed Audio Recaps" data="The current draft recap script and speech-generation request metadata; the speech step does not receive uploaded document files or research queries" status={config.speechConfigurationError ? 'Configuration incomplete' : config.speechEnabled ? 'Enabled' : 'Disabled'} />
