...
- **Petrios Recall (spec/08)**: AI-drafted recall questions (gateway purpose `recall_questions`), moderator-approved on the session manage Recall tab, emailed via `recall-send`. The Recall tab starts with the separately approved, uploaded-document-led and authoritatively researched ~5-minute Audio Recap workflow, including estimated generation progress and moderator-visible research sources; it also shows aggregate-only retention analytics (cohorts under five suppressed; never an assistant tool). Passing catch-up records learning completion but does not change policy-v2 physical attendance or certificate eligibility. Public answer page `/recall/[token]` uses an HMAC capability.
